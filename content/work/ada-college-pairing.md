...
Within the basic principle of two (or more) people working on the same task 
simultaneously, there are many specific variations. Here are a couple you can
try out:

  - **Driver-navigator**: the classic pairing style sees one person 
    "driving", i.e. actually typing out the code, and the other "navigating",
     suggesting where to go next. This allows one person to focus on the 
     detail and one on the bigger picture. Roles switch frequently, e.g. in 
     the "ping-pong" style one person gets a failing test to pass, writes a 
     new test that fails, then hands over to their partner.
